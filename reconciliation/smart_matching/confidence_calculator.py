...
class ConfidenceCalculator:
    ...
    def __init__(self):
        self.engine_weights = {
            ...
            "Mongolian Name Engine": 0.85,  # High confidence for name matching
        }
    ...
    def calculate_weighted_confidence(self, suggestions: List[Dict]) -> List[Dict]:
        ...
        for suggestion in suggestions:
            ...
            method_bonus = self._get_method_confidence_bonus(
    ...
    def calculate_ensemble_confidence(
        self, suggestions_by_engine: Dict[str, List[Dict]]
    ) -> Dict:
        ...
        for engine_name, suggestions in suggestions_by_engine.items():
            ...
            if weighted_suggestions:
                ...
                best_suggestion = max(
                    weighted_suggestions, key=lambda x: x["confidence"]
                )
        ...
        ensemble_result = {
            ...
            "confidence_std": (
                round(statistics.stdev(confidences)) if len(confidences) > 1 else 0
            ),
    ...
    def calibrate_confidence(self, suggestion: Dict, actual_outcome: bool) -> Dict:
        ...
        return {
            ...
            "calibrated_confidence": max(
                1, min(99, predicted_confidence + calibration_adjustment)
            ),
    ...
    def _calculate_calibration_adjustment(self, engine_name: str, method: str) -> float:
        ...
        relevant_feedback = [
            f for f in feedback_entries if f["method"] == method or method == ""
        ]
    ...
    def get_engine_performance_stats(self) -> Dict:
        ...
        for engine_name, feedback_entries in self.feedback_data.items():
            ...
            correct_predictions = sum(
                1 for f in feedback_entries if f["actual_outcome"]
            )
            ...
            avg_confidence = statistics.mean(
                f["predicted_confidence"] for f in feedback_entries
            )
            ...
            stats[engine_name] = {
                ...
                "last_calibration": (
                    max(f["timestamp"] for f in feedback_entries)
                    if feedback_entries
                    else None
                ),
            }
    ...
    def apply_weight_adjustments(self, adjustments: Dict):
        ...
        for engine_name, adjustment in adjustments.items():
            if engine_name in self.engine_weights:
                ...
                print(
                    f"Updated {engine_name} weight: {old_weight:.2f} → {new_weight:.2f}"
                )
    ...
    def _track_confidence_history(self, engine_name: str, confidence: float):
        ...
        if len(self.confidence_history[engine_name]) > 1000:
            self.confidence_history[engine_name] = self.confidence_history[engine_name][
                -1000:
            ]
    ...
    def get_confidence_trends(self, engine_name: Optional[str] = None) -> Dict:
        ...
        if engine_name:
            ...
            return {
                ...
                "average_confidence": (
                    round(statistics.mean(confidences), 1) if confidences else 0
                ),
                "confidence_trend": self._calculate_trend(confidences),
                "last_10_average": (
                    round(statistics.mean(confidences[-10:]), 1)
    ...
    def export_calibration_data(self) -> Dict:
        ...
        return {
            ...
            "total_feedback_entries": sum(
                len(feedback) for feedback in self.feedback_data.values()
            ),
        }
        ...
        if "feedback_data" in data:
            for engine, feedback_list in data["feedback_data"].items():
                for feedback in feedback_list:
                    ...
                    feedback_entry["timestamp"] = datetime.fromisoformat(
                        feedback["timestamp"]
                    )
                    self.feedback_data[engine].append(feedback_entry)

        print(
            f"Imported calibration data for {len(data.get('feedback_data', {}))} engines"
        )

...
class SmartMatchingRegistry:
    ...
    def _register_default_engines(self):
        """Register the remaining 3 engines"""
        try:
            from .engines.license_plate_engine import LicensePlateEngine

            self.register_engine(LicensePlateEngine)
        except ImportError:
            print("❌ LicensePlateEngine not available")
        ...
        try:
            ...
            print("❌ PhonePriorityEngine not available")
        ...
        try:
            ...
            print("❌ IDPriorityEngine not available")
    ...
    def get_best_suggestions(
        self, bank_description: str, amount: float, max_results: int = 5
    ) -> List[Dict]:
        ...
        for suggestion in all_suggestions:
            ...
            if loan_id:
                if (
                    loan_id not in unique_suggestions
                    or suggestion["confidence"]
                    > unique_suggestions[loan_id]["confidence"]
                ):
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "summary": {
                ...
                "enabled_engines": sum(
                    1 for engine in self.engines.values() if engine.enabled
                ),
                "disabled_engines": sum(
                    1 for engine in self.engines.values() if not engine.enabled
                ),
            },
        }
